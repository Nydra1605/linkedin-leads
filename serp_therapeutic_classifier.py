...
def classify_company_with_llm(context: str, source_urls: list, company_name: str) -> dict | None:
    ...
    prompt = f"""
    Based on the provided text about the company "{company_name}", classify whether the company is working on each of the following therapeutic categories.
    
    For each category, respond with "Yes" if the company is working on that modality, or "No" if there's no evidence they are working on it.
    ...
    {{
        "small_molecules": "Yes" or "No",
        "peptides": "Yes" or "No",
        "proteins": "Yes" or "No",
        "antibodies": "Yes" or "No",
        "antibody_conjugates": "Yes" or "No",
        "fusion_proteins": "Yes" or "No",
        "nucleic_acid_therapeutics": "Yes" or "No",
        "gene_cell_therapies": "Yes" or "No",
        "vaccines": "Yes" or "No",
        "protein_degraders": "Yes" or "No",
        "other_modalities": "Yes" or "No"
    }}
    
    Important: Only return "Yes" if there is clear evidence the company is actively developing or has developed therapeutics in that category. If uncertain or no information is found, return "No".
    """
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a precise pharmaceutical company classifier that analyzes companies and determines which therapeutic modalities they are working on. You only respond with 'Yes' or 'No' for each category based on clear evidence. Return your response as a valid JSON object only."},
...
def save_to_csv(data: dict, company_name: str, filename: str = "therapeutic_classification_output.csv"):
    """Appends the classified company data to a CSV file."""
    # Define all fieldnames including company_name and all categories
    fieldnames = ['company_name'] + list(THERAPEUTIC_CATEGORIES.keys())
...
def process_company_data(company_name: str, output_csv_file: str):
    """
    Main function to process search results for a single company.
    It searches, scrapes all links, classifies the company, and saves the result.
    """
    ...
    if not unique_urls:
        ...
        # Save a record with "No" for all categories
        default_data = {category: "No" for category in THERAPEUTIC_CATEGORIES.keys()}
        save_to_csv(default_data, company_name, output_csv_file)
        return
    ...
    # 4. Classify the company using LLM
    classification_data = classify_company_with_llm(full_context, unique_urls, company_name)

    if classification_data:
        # 5. Save the classification results to a CSV file
        save_to_csv(classification_data, company_name, output_csv_file)
    else:
...
def process_companies_from_csv(input_csv_file: str, output_csv_file: str = "therapeutic_classification_output.csv"):
    """
    Reads company names from a CSV file and processes each one for classification.
    ...
    try:
        ...
        all_companies = df['company_name'].dropna().unique()
        ...
        for idx, company in enumerate(companies_to_process, 1):
            ...
            try:
                process_company_data(company, output_csv_file)
            except Exception as e:
                print(f"❌ Error processing {company}: {e}")
                # Save error entry to maintain progress tracking
                error_data = {category: "Error" for category in THERAPEUTIC_CATEGORIES.keys()}
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV_FILE = "therapeutic_classification_output.csv"  # Output file name
